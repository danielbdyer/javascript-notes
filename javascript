...
-	Arithmetic operators apply here. One new one: % -- remainder. 314 % 100 = 14.
...
-	var stringVariable = “Dog” can have bracket notation to call a character, i.e.,
		stringVariable[0]; returns “D”
			finding the last character? [stringVariable[stringVariable.length - 1]
...
Arrays:
-	Multi-dimensional array: [[“Bulls”, 23], [“White Sox”, 45]]
-	teamScore[0,1]; returns 23
-	Arrays can be updated/altered by doing teamscore[0][1] = 24;
...
-	.push(para1) or ([para2, paran] takes one or more parameters and pushes them to the end of an array

	var arr = [1,2,3];
	arr.push(4);
	// arr is now [1,2,3,4]

-	.pop() functions pop the last entry – any kind of entry, whether number, string, or another nested array – off the array. var oneDown = [1, 4, 6].pop(); return oneDown; “6”
...
-	Thinking of a queue as an abstract data structure – keeping items in order. New items added at the back, old items removed from the front.
...
Objects are similar to arrays, except that instead of using indexes to access and modify their data, you access the data in objects through what are called properties. A Good use for Objects: Lookup Tables!
...
If the property of the object has a space, it is accessible by bracket notation. myObj[“Property Two”](/single quote)
...
myObj[“propName”] = “New string one”; or myObj.propNameTwo = “New string two”;
and delete it by using something like delete myObj.propNameTwo; .
Object properties can be checked re: existing (i.e., propName) by using .hasOwnProperty method, i.e., myObj.hasOwnProperty(“propName”)
...
Variables:
Variables can be local (inner-function) or global (initialized outside of a function). If there are conflicting variable definitions in a function, between local and global types, local takes precedence.
...
for loops (in most cases are shorter and clearer than a while construct) i++, i += n, or i -= n
initialize loop (define variable); check loop (conditional logic); update loop (delta for each iteration)
...
switches have to have breaks, otherwise you will just also execute the next “switch” case in the code. Sometimes you may want this – you just need to be careful about it. default case at the end of it.
...
Regex
regex101.com/ a great tester and can produce a lot of basic regex strings by default
